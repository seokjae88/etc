...
class HeaderPacket {
private:
    int code_; // 101:req_login, 102:req_download, 201:res_login, 202:res_download, 999:error
    int size_; // data size (total - header)
    int errCode_; // error code
    int crc_;
public:
    HeaderPacket() : code_(0), size_(0), errCode_(0), crc_(0) {
    }
